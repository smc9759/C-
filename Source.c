	...
	for (int i = 0; i < row; i++)
	{
		for (int j = 0; j < col; j++)
		{
			if (matrix[i][j] == '*')
			{
				...
				if (j - 1 >= 0 && i - 1 >= 0 && matrix[i - 1][j - 1] != '*') matrix[i - 1][j - 1] += 1; //좌상
				if (i + 1 < row && matrix[i + 1][j] != '*') matrix[i + 1][j] += 1;//하
				if (j + 1 < col && i + 1 < row && matrix[i + 1][j + 1] != '*') matrix[i + 1][j + 1] += 1;//우하
				if (j - 1 >= 0 && i + 1 < row && matrix[i + 1][j - 1] != '*') matrix[i + 1][j - 1] += 1;//좌하
				if (j + 1 < col && i - 1 >= 0 && matrix[i - 1][j + 1] != '*') matrix[i - 1][j + 1] += 1;//우상
			}
		}
	}
